...
- For showing in-progress work on a phone or to anyone outside this
  machine, expose the dev server via a Cloudflare quick-tunnel:

  ```sh
  DEV_TUNNEL=1 npm run dev   # binds 0.0.0.0, allows any Host header
  cloudflared tunnel --url http://localhost:5173
  ```

  `scripts/setup.sh` (also the container Setup script) installs
  `cloudflared`. The Claude Code on the web environment must be set to
  **Full** network access — Trusted blocks `trycloudflare.com`.
